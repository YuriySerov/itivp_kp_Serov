...
if (!$plant) die("Растение не найдено.");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$errors) {
        $stmt = $pdo->prepare("UPDATE plants SET name=?, description=?, watering_schedule=?, last_watered=? WHERE id=?");
        $stmt->execute([$name, $description, $watering_schedule, $last_watered, $id]);
...
<h1 class="mb-4">✏️ Редактировать растение</h1>
